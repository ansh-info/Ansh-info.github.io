...
const DockerDashboard = ({ isMobile = false, isOpen, setIsOpen }) => {
  ...
  const toggleContainer = (id) => {
    setContainers((prev) =>
      prev.map((container) => {
        if (container.id === id) {
          const newStatus =
            container.status === "running" ? "sleeping" : "running";
          addLog(
            `Container ${container.name} ${newStatus === "running" ? "started" : "stopped"}`,
          );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (Math.random() < 0.3) {
        ...
        addLog(
          `[${type}] ${messages[Math.floor(Math.random() * messages.length)]}`,
        );
      }
    }, 3000);
  ...
  return (
    ...
    >
      ...
      {/* System Stats */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 p-4 border-b border-gray-700 shrink-0">
        {[
          {
      ...
      <div className="p-4 border-t border-gray-700 shrink-0">
        ...
        <div className="bg-gray-900/50 rounded-lg p-2 font-mono text-xs space-y-1 h-[100px] overflow-y-auto">
          {logs.map((log, index) => (
            <div
              key={index}
              className={`${
